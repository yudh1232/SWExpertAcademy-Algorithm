...
def set_wire(core_number, connected_core, wire_length, wire):
    ...
    x = cores[core_number][0]
    y = cores[core_number][1]
    ...
    # 가장자리에 있지 않은 코어
        ...
        for d in range(4):
            count = 0
            nx = cores[core_number][0]
            ny = cores[core_number][1]
            # 전선 설치
            while True:
                nx += dx[d]
                ny += dy[d]
                if nx < 0 or nx >= n or ny < 0 or ny >= n:
                    set_wire(core_number + 1, connected_core + 1, wire_length + count, wire)
                    break
                elif board[nx][ny] == 1 or wire[nx][ny] == 1:
                    break
                wire[nx][ny] = 1
                count += 1
